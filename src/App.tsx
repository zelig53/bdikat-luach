  ...
  ({ day, targetDate, theme, hasNotes, onClick }) => {
    ...
    return (
      <div className={`relative aspect-square ${day.date > targetDate ? 'opacity-10 grayscale' : ''}`}>
        ...
        {/* Dots outside overflow-hidden — always pinned to corner */}
        {day.hasidicEvent && (
          <div className={`absolute top-0.5 left-0.5 w-1.5 h-1.5 rounded-full border-2 ${theme.hasidicDot} opacity-75 pointer-events-none`} />
        )}
        {hasNotes && (
          <div className={`absolute bottom-0.5 right-0.5 w-1.5 h-1.5 rounded-full border-2 ${theme.notesDot} opacity-75 pointer-events-none`} />
        )}
      </div>
    );
  };
