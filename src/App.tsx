...
async function showNotification(title: string, body: string, tag?: string) {
  ...
  // נסה Notification ישירה קודם — הכי אמין כשהאפליקציה פתוחה
  try {
                ...
                <div className="p-6 space-y-7 max-h-[75vh] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    ...
                    <div className={`${theme.cardBg} rounded-xl p-4 border ${theme.cardBorder} space-y-3`}>
                      ...
                      <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                        ...
                        <button onClick={async () => {
                          const granted = await requestNotificationPermission();
                          setNotifPermission(Notification.permission);
                          if (!granted) { alert('הרשאת התראות לא ניתנה — בדוק הגדרות הדפדפן'); return; }
                  ...
                  <div className="space-y-3">
                    ...
                    {isInstallable ? (
                      ...
                      <div className={`${theme.cardBg} rounded-xl p-4 border ${theme.cardBorder} text-center`}>
                        <p className="text-xs font-black text-green-600">✅ האפליקציה מותקנת במכשיר</p>
                      </div>
                    )}
                  </div>
